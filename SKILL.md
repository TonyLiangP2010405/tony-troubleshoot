...
6. 范围内每种型号、系列及固件/API 版本所对应的准确 API 命令文档，可为本地文件、URL 或用户粘贴的完整内容。
...
不得凭记忆猜测命令或拿其他型号手册代替。文档缺失、型号/API 主版本不匹配或适用性不明时，不得向受影响设备发送命令；应把缺口登记为阻塞。只有满足本 Skill 的“小版本 API 兼容性探测”条件时，才可解锁已逐条验证的少量只读命令。`audit` 模式存在任何文档缺口时不得声称全量完成。
...
2. 读取该目录的 `session-handoff.md`、`checkpoint.json`、`intake.md`、`user-actions.csv`、`source/customer-file-index.csv`、`customer-file-review.md`、`baseline-index.csv`、`baseline-comparison.csv`、`wyrestorm-official-research.md`、`classification.md`、文档索引、各 CSV 和 `checkpoint-log.md` 末尾。
...
1. 完成 intake、客户文件详细审阅和定向官网预检，登记已有故障前基线；没有可靠基线时计划在首次设备动作前采集 `session_start` 只读快照。给出有依据的初始分类并建立文档索引和必要的 `core` 命令目录。
2. 发现设备、形成 API 配置组，并保存发现证据。
3. 对所有在线设备运行一次核心健康普查，并保存可比较的会话开始快照；它只作为后续动作的锚点。
