...
description: "根据用户描述、已执行操作、详细审阅的客户文件、当前型号/固件/症状的 WyreStorm 官方资料和准确 API 命令文档，对单台或多台 WyreStorm 设备进行分类和分层故障排查；先记录操作并验证客户文件阅读覆盖，再按七类问题定位。检查点仅供同一会话续查，客户确认解决后只保留一份结案总结。缺少适用文档时不得连接设备。"
---
...
`intake_status=complete` 后，先读取 [客户文件详细审阅规则](references/customer-file-review.md)，把每个客户提供的文件登记到 `source/customer-file-index.csv`，并把逐文件阅读结果写入 `customer-file-review.md`。不能只看文件名、缩略图、摘要、搜索命中、第一页或日志尾部就声称已读完。
...
没有收到文件时标记 `customer_file_review_status=none_provided`。已收到的相关文件全部完成覆盖检查后才能标记 `complete`；文件损坏、加密、缺页或关键内容无法可靠读取时标记 `blocked`，说明需要客户重新导出、解密或改用可读格式。只有 `complete|none_provided` 才能继续初始分类；不得用低置信度措辞掩盖未读完的高影响文件。

## WyreStorm 官方资料预检

`intake_status=complete` 且客户文件审阅门通过后、初始分类和设备连接前，根据本次准确型号、系列、固件和症状执行一次定向官网研究。读取 [WyreStorm 官方资料预检规则](references/wyrestorm-official-preflight.md)，并把结果写入 `wyrestorm-official-research.md`。

- 只研究范围内型号及信号链直接相关的内容，不泛读整个官网，也不把其它系列的资料套用到当前设备。
- 优先核对产品页、规格、Quick Start/User Manual、固件/软件/驱动及发行说明、API/控制文档、兼容性表、交换机/网络配置指南和症状相关支持文章。
- 记录准确 URL、访问时间、适用型号/版本、确认事实、推论和对诊断选择的影响；事实与推论必须分开。
- 官网无法访问时将 `official_research_status` 标为 `unavailable` 并记录原因，可以依据用户提供的准确文档继续；不得假装已完成官网核对。
- 官网背景资料不能替代准确且适用的 API 命令文档。即使官网存在 API 文件，版本或型号适用性不明时仍不得向设备发送命令。

只有 `official_research_status=complete|unavailable` 后才能给出初始分类。官网研究是只读资料核对，不授权下载后安装固件/软件，也不授权更改设备。
...
- `wyrestorm-official-research.md`：当前型号/固件/症状的官网预检状态、来源、适用性、事实和诊断影响。
...
2. 读取该目录的 `session-handoff.md`、`checkpoint.json`、`intake.md`、`user-actions.csv`、`source/customer-file-index.csv`、`customer-file-review.md`、`wyrestorm-official-research.md`、`classification.md`、文档索引、各 CSV 和 `checkpoint-log.md` 末尾。
3. 核对 checkpoint 中的 nonce 与当前会话保存的值一致，再核对 intake、客户文件清单及阅读覆盖、官网预检、分类、`scan_mode`、目标范围、文档指纹、设备身份和 API 配置；不一致时停止续查。
...
1. 完成 intake、客户文件详细审阅和定向官网预检，给出有依据的初始分类并建立文档索引和必要的 `core` 命令目录。
...
完成 intake、客户文件详细审阅、定向官网预检和初始分类后，围绕指定设备或问题域建立最小充分命令集。沿依赖链收集支持与反证，避免无关端口、页面和参数组合。若发现故障可能跨设备传播，可把受影响链路设备加入范围，但要记录加入原因。

### `audit`：全量覆盖

完成 intake、客户文件详细审阅、定向官网预检和初始分类；分类不缩减审计范围，但用于报告问题上下文。完整读取文档并编制每条命令。对每台适用在线设备穷尽所有有限参数、页码和游标，保存全部响应并审计每个预期字段。汇总命令只有在文档明确证明与详细命令等价时才能替代详细查询。
...
`findings.md` 先写有证据路径的确认事实，再写诊断判断和反证。`final-report.md` 必须包含用户已执行操作摘要、客户文件清单与阅读覆盖证明、文件中的关键事实/矛盾/未读部分及其影响、与结论有关的官网来源和适用性、最终主/次分类及置信度、分类变化原因、模式、范围、时间和时区、文档/版本指纹、设备与组别数量、执行/失败/阻塞统计、异常设备及深入理由、脱敏情况和未解决缺口。
