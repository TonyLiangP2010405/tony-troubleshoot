...
description: "根据用户描述、已执行操作、当前型号/固件/症状的 WyreStorm 官方资料和准确 API 命令文档，对单台或多台 WyreStorm 设备进行分类和分层故障排查：先记录用户点过、改过和插拔过的内容，再按七类问题定位；检查点仅供同一会话在模型切换或上下文压缩后续查，客户确认解决后只保留一份结案总结。缺少适用文档时不得连接设备。"
---
...
`intake_status=complete` 后、初始分类和设备连接前，根据本次准确型号、系列、固件和症状执行一次定向官网研究。读取 [WyreStorm 官方资料预检规则](references/wyrestorm-official-preflight.md)，并把结果写入 `wyrestorm-official-research.md`。
...
2. 读取该目录的 `session-handoff.md`、`checkpoint.json`、`intake.md`、`user-actions.csv`、`wyrestorm-official-research.md`、`classification.md`、文档索引、各 CSV 和 `checkpoint-log.md` 末尾。
3. 核对 checkpoint 中的 nonce 与当前会话保存的值一致，再核对 intake、官网预检、分类、`scan_mode`、目标范围、文档指纹、设备身份和 API 配置；不一致时停止续查。
...
1. 完成 intake 和定向官网预检，给出有依据的初始分类并建立文档索引和必要的 `core` 命令目录。
...
完成 intake、定向官网预检和初始分类后，围绕指定设备或问题域建立最小充分命令集。沿依赖链收集支持与反证，避免无关端口、页面和参数组合。若发现故障可能跨设备传播，可把受影响链路设备加入范围，但要记录加入原因。

### `audit`：全量覆盖

完成 intake、定向官网预检和初始分类；分类不缩减审计范围，但用于报告问题上下文。完整读取文档并编制每条命令。对每台适用在线设备穷尽所有有限参数、页码和游标，保存全部响应并审计每个预期字段。汇总命令只有在文档明确证明与详细命令等价时才能替代详细查询。
...
`findings.md` 先写有证据路径的确认事实，再写诊断判断和反证。`final-report.md` 必须包含用户已执行操作摘要、与结论有关的官网来源和适用性、最终主/次分类及置信度、分类变化原因、模式、范围、时间和时区、文档/版本指纹、设备与组别数量、执行/失败/阻塞统计、异常设备及深入理由、脱敏情况和未解决缺口。
